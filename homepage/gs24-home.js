// Department URLs for credits
const gs24DeptURLs = {
	"Architecture": "https://publications.risdmuseum.org/grad-show-2024-architecture",
	"Ceramics": "https://publications.risdmuseum.org/grad-show-2024-ceramics",
	"Design Engineering": "https://publications.risdmuseum.org/grad-show-2024-design-engineering",
	"Digital + Media": "https://publications.risdmuseum.org/grad-show-2024-digital-media",
	"Furniture Design": "https://publications.risdmuseum.org/grad-show-2024-furniture-design",
	...
	"Landscape Architecture": "https://publications.risdmuseum.org/grad-show-2024-landscape-architecture",
	"Painting": "https://publications.risdmuseum.org/grad-show-2024-painting",
	"Photography": "https://publications.risdmuseum.org/grad-show-2024-photography",
	"Printmaking": "https://publications.risdmuseum.org/grad-show-2024-printmaking",
	"Sculpture": "https://publications.risdmuseum.org/grad-show-2024-sculpture",
	"Teaching + Learning in Art + Design": "https://publications.risdmuseum.org/grad-show-2024-teaching-learning-art-design",
...
function generateImages() {	
	...
	for (let col=0; col<totalDivisions; col++) {
		...
		for (let row=0; row<rows; row++) {
			...
			let slicesTemp = '';
			const randomImageIndex = Math.floor(Math.random()*gs24Images.length);
			const randomImage = gs24Images[randomImageIndex]["img"];

			for (let slice=0; slice<slices; slice++) {
				slicesTemp += `
					<div class="gs24-home-genart-slice" style="background-image:url('/assets/student-work/${randomImage}'); animation: background-shift 600s -${Math.random()*10}s infinite linear;" data-index="${randomImageIndex}">
...
function addCredit(slice) {
	if (mode == 'interactive') {
		let imageIndex = parseInt(slice.dataset.index);
		homeCredit.innerHTML = `
			<a class="gs24-home-credit-name" href="${gs24Images[imageIndex]["page"]}">
				...
				<span>${gs24Images[imageIndex]["name"]}</span>
			</a>
			<a class="gs24-home-credit-dept" href="${gs24DeptURLs[gs24Images[imageIndex]["dept"]]}">
				<svg viewBox="0 0 100 100"><path d="M40,0h20v10h10v10h10v10h10v10h10v20h-10v10h-10v10h-10v10h-10v10h-20v-20h10v-10h10v-10H0v-20h60v-10h-10v-10h-10V0Z"/></svg>
				<span>${gs24Images[imageIndex]["dept"]}</span>
			</a>
		`;
...
function homeFeature() {
	...
	let imageIndex = parseInt(activeHomeImage.dataset.index);
	homeCredit.innerHTML = `
		<a class="gs24-home-credit-name" href="${gs24Images[imageIndex]["page"]}">
			<svg viewBox="0 0 100 100"><path d="M40,0h20v10h10v10h10v10h10v10h10v20h-10v10h-10v10h-10v10h-10v10h-20v-20h10v-10h10v-10H0v-20h60v-10h-10v-10h-10V0Z"/></svg>
			<span>${gs24Images[imageIndex]["name"]}</span>
...
function generateTitleBackground() {
	...
	for (let col=0; col<Math.round(200/1.25); col++) {
		homeGenartTemp += `
			<div class="gs24-home-title-background-col" style="background-image: url('/assets/student-work/${gs24Images[Math.floor(Math.random()*gs24Images.length)]["img"]}'); animation: background-shift 600s -${Math.random()*10}s infinite linear;">
				<div class="gs24-home-genart-slice-overlay"></div>
			</div>
		`;
	}
...
function generateTOC() {
	for (let tocLink of document.querySelectorAll('.gs24-toc-link')) {
		...
		tocLink.innerHTML += `
			<div class="gs24-toc-background" style="background-image: url('/assets/student-work/${gs24Images[Math.floor(Math.random()*gs24Images.length)]["img"]}'); animation: background-shift 600s -${Math.random()*10}s infinite linear;"></div>
		`;
	}
}
function regenerateTOC() {
	for (let tocLinkBackground of document.querySelectorAll('.gs24-toc-background')) {
		tocLinkBackground.style.backgroundImage = `url('/assets/student-work/${gs24Images[Math.floor(Math.random()*gs24Images.length)]["img"]}')`;
	}
}
